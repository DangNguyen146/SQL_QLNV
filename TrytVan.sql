...
SELECT SP.MASP, SP.TENSP, SP.NUOCSX  FROM SANPHAM AS SP WHERE MASP LIKE 'B%01'
...
SELECT SOHD, TRIGIA FROM HOADON WHERE HOADON.NGHD IN('20070101', '20070102') 
--7 In ra các số hóa đơn, trị giá hóa đơn trong tháng 1/2007, sắp xếp theo ngày (tăng dần) và trị giá của hóa đơn (giảm dần).
SELECT SOHD, TRIGIA FROM HOADON WHERE YEAR(HOADON.NGHD)=2007 AND MONTH(HOADON.NGHD)=1 ORDER BY NGHD ASC, TRIGIA DESC
--8 In ra danh sách các khách hàng (MAKH, HOTEN) đã mua hàng trong ngày 1/1/2007.
SELECT KH.MAKH, HOTEN FROM dbo.KHACHHANG AS KH, dbo.HOADON AS HD WHERE KH.MAKH=HD.MAKH AND HD.NGHD='20070101'
--9 In ra số hóa đơn, trị giá các hóa đơn do nhân viên có tên “Nguyen Van B” lập trong ngày 28/10/2006.
SELECT COUNT(*) FROM dbo.HOADON, dbo.NHANVIEN WHERE NHANVIEN.HOTEN='Nguyen Van B' AND HOADON.MANV=NHANVIEN.MANV AND HOADON.NGHD='20061028'
--10 In ra danh sách các sản phẩm (MASP,TENSP) được khách hàng có tên “Nguyen Van A” mua trong tháng 10/2006.
SELECT SANPHAM.MASP , TENSP
FROM (KHACHHANG JOIN HOADON ON KHACHHANG.MAKH = HOADON.MAKH) JOIN ( SANPHAM JOIN CTHD ON SANPHAM.MASP = CTHD.MASP ) ON CTHD.SOHD = HOADON.SOHD
...
SELECT *
FROM KHACHHANG KH, HOADON HD, SANPHAM SP, CTHD CT
WHERE KH.MAKH=HD.MAKH AND SP.MASP = CT.MASP AND HD.SOHD=CT.SOHD AND HOTEN='Nguyen Van A' AND YEAR(NGHD)=2006 AND MONTH(NGHD)=10